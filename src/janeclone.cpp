...
void JaneClone::SetProperties() 
{
     ...
     wxString jc = wxPathCombine(::wxGetHomeDir(), JANECLONE_DIR);
     wxDir jcDir(jc);
...
{
     ::wxMkdir(wxPathCombine(jcDir.GetName(), wxT("dat")));
     ::wxMkdir(wxPathCombine(jcDir.GetName(), wxT("prop")));
     ::wxMkdir(wxPathCombine(jcDir.GetName(), wxT("cache")));
}
/**
 * DoLayout
...
{     
     ...
     const wxString prop = wxPathCombine(::wxGetHomeDir(), JANECLONE_DIR, APP_CONFIG_FILE);
...
void JaneClone::SetPreviousUserLookedTab() 
{
     ...
     for (unsigned int i = 0; i < userLookedBoardList.GetCount(); i++) 
     {
	  ...
	  if (!boardNameAscii.IsEmpty()) 
	  {
	       ...
	       wxString outputPath = wxPathCombine(::wxGetHomeDir(),
						   JANECLONE_DIR,
						   wxT("dat"),
						   boardNameAscii,
...
{
     // ファイルパスの組み立て
     wxDir jcDir(wxPathCombine(::wxGetHomeDir(), JANECLONE_DIR));
     wxString filePath = wxPathCombine(jcDir.GetName(), wxT("dat"));
